...
public class AnalyticsReporterProcessor extends AbstractProcessor {
    ...
    private List<TypeElement> findTypesAnnotatedWIthReporter(Set<? extends Element> annotatedElements) {
        ...
        for (Element element : annotatedElements) {
            if (element.getKind() != ElementKind.INTERFACE || !(element instanceof TypeElement)) {
                messager.printMessage(Diagnostic.Kind.ERROR, "@Reporter is available only for Interface");
            } else {
    ...
    private void saveTypeSpec(TypeSpec typeSpec, TypeElement typeElement) {
        try {
            SaveUtils.saveClass(typeElement.getEnclosingElement().getClass().getPackage().getName(), typeSpec, processingEnv.getFiler());
        } catch (IOException e) {
